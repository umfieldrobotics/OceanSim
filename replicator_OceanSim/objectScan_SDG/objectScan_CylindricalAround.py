...
async def run_scan_async(cam, r0, z_range, azi_range, num_data):
    i = 0 
    while i < num_data:
        z = np.random.uniform(z_range[0], z_range[1])
        azi = np.random.uniform(azi_range[0], azi_range[1])

        with cam:
            rep.modify.pose(
                position = [r0 * np.cos(azi), r0 * np.sin(azi), z ],
                look_at = [0, 0, z]
            )
...
asyncio.ensure_future(run_scan_async(cam, r0, z_range, azi_range, num_data))
