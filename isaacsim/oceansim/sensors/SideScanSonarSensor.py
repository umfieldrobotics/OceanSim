...
class SideScanSonarSensor(Camera):
    def __init__(self, 
                ...
                #  min_range: float = 0.2, # m assume range on the left half of the scan plane has negative range, thus range falls in [-max_range, max_range]
                 max_range: float = 3.0, # m
                 range_res: float = 0.008, # deg
                 hori_fov: float = 130.0, # deg
                 vert_fov: float = 20.0, # deg
                #  angular_res: float = 0.5, # deg No azimuth resolution in side scan sonar
        ...
        #                                indexing='ij')
        r = np.arange(-self.max_range, self.max_range, self.range_res)
