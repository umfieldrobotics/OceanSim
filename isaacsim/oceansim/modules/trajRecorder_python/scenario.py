...
class Colorpicker_Scenario():
    def __init__(self):
        ...
        self.freq = 10
    def setup_scenario(self, freq:int):
        self.freq = freq

        self._running_scenario = True
        self._viewport_cam = ViewportCameraState()
    ...
    def teardown_scenario(self):
        ...
        self._id = 0
        self.recorded_position = []
        self._recorded_size = 0


    def update_scenario(self, step: float, recorded_size_display):

        
        if not self._running_scenario:
            return
        
        viewport_position = self._viewport_cam.position_world
        if self._id >= self.freq:
            self.recorded_position.append([*viewport_position])
            self._recorded_size = len(self.recorded_position)
            recorded_size_display.set_value(self._recorded_size)
            self._id = 0
