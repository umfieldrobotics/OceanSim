#!/bin/bash
# Bash script to run UWCam_sdg_COU.py on all YAML configs in a specified config directory
...
SCRIPT="${BASEDIR}extsUser/isaacsim.oceansim/standalone/UWCam_sdg_COU.py"
TIMES_LOG_FILE="${SCRIPT_DIR}/run_sdg_COU_times.txt"

# Use first argument as config directory, or default if not provided
if [ -z "$1" ]; then
    CONFIG_DIR="${BASEDIR}extsUser/isaacsim.oceansim/standalone/COU_configs"
else
