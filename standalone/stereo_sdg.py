########################################
#### Here goes setup simulation app ####
########################################
...
from omni.replicator.core import Writer, AnnotatorRegistry, BackendDispatch, rng


__version__ = "0.0.1"

class StereoCamWriter(Writer):
    def __init__(
        self,
        output_dir,
        ...
        # distance to image plane annotator
        self.annotators.append(AnnotatorRegistry.get_annotator(
            "distance_to_image_plane"))
    
        # camera parameters annotator
        self.annotators.append(AnnotatorRegistry.get_annotator(
            "camera_params"
        ))
    ...
    def write_metadata(self):
        self.metadata = {
            ...
            "replicator_global_seed:": rng.get_global_seed(),
            "UW_render_param" : self._UW_param.tolist()
        }
        # this saves the metadata that will be used to read the file
        self._backend.schedule(F.write_json, data=self.metadata, path="metadata.txt")
        self._is_metadata_written = True 

    def _write_rgb(self, data, sub_dir: str, rgb_annotator: str, dist_to_cam_annotator: str):
        # Save the rgb data under the correct path
        uw_image = wp.empty(shape=(self._resolution[1], self._resolution[0], 4), dtype=wp.uint8)
        rgb_dir_name = "UW_rgb"
        ...
        self._backend.schedule(F.write_image, data=uw_image, path=rgb_file_path)
    
    def _write_depth(self, data, sub_dir: str, annotator: str):
        depth_dir_name = 'depth'
        depth_file_path = os.path.join(sub_dir, depth_dir_name, f'depth_{self._frame_id}.npy')
        self._backend.schedule(F.write_np, data=data[annotator], path=depth_file_path)
    
    
    def _write_camera_parameters(self, data, sub_dir:str, camera_params_annotator:str, _format: str = None):
        camera_param_dir_name = 'camera_param'
        camera_param_file_path = os.path.join(sub_dir, camera_param_dir_name, f'cam_param_{self._frame_id}.json')
        camera_params = data[camera_params_annotator]
        camera_data = {}
        if _format is None:
        ...
        self._backend.schedule(F.write_json, data=camera_data, path=camera_param_file_path)
    ...
    def write(self, data):
        ...
        for render_product in render_products:
            ...
            self._write_rgb(data, sub_dir, f"rgb-{render_product_name}", f"distance_to_camera-{render_product_name}")
            self._write_depth(data, sub_dir, f"distance_to_image_plane-{render_product_name}")
            self._write_camera_parameters(data, sub_dir, f"camera_params-{render_product_name}")


        self._frame_id += 1


WriterRegistry.register(StereoCamWriter)
...
def main():
    ...
    # Here you can adjust the settings of the stereo camera, link to API:
    # https://docs.omniverse.nvidia.com/py/replicator/1.5.1/source/extensions/omni.replicator.core/docs/API.html?highlight=stereo%20camera#omni.replicator.core.create.stereo_camera
    baseline = 0.5  # meters
    stereo = rep.create.stereo_camera(stereo_baseline=baseline)
    ...
    # Set up the writer
    writer = rep.WriterRegistry.get("StereoCamWriter")
    ...
    render_product = rep.create.render_product(stereo, RESOLUTION)
